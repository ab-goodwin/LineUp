...
function BuddyCard({ buddy, onRemove }: {
      ...
      {pendingRequests.length > 0 && (
        <div className="mb-6 bg-primary/5 border-2 border-primary/20 rounded-2xl p-4 space-y-3">
          ...
          {pendingRequests.map(req => (
            <div
              key={req.id}
              className="flex items-center justify-between gap-3 bg-card rounded-xl px-4 py-3 border border-border"
            ...
            >
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() => handleRespond(req.id, "accept", req.requesterFirstName)}
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  className="rounded-lg h-8 text-muted-foreground hover:text-destructive"
                  onClick={() => handleRespond(req.id, "decline", req.requesterFirstName)}
      ...
      <Tabs defaultValue="buddies" className="space-y-4">
        <TabsList className="w-full bg-secondary/40 rounded-xl">
          <TabsTrigger value="buddies" className="flex-1 rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm">
            Buddies
            {buddyList.length > 0 && (
              <span className="ml-1.5 text-xs bg-primary/15 text-primary rounded-full px-1.5">{buddyList.length}</span>
            )}
          </TabsTrigger>
        ...
        <TabsContent value="buddies" className="space-y-3 mt-2">
          {buddiesLoading ? (
            ...
            <AnimatePresence mode="popLayout">
              {buddyList.map(buddy => (
                <BuddyCard key={buddy.userId} buddy={buddy} onRemove={handleRemove} />
              ))}
            </AnimatePresence>
          )}
        </TabsContent>
